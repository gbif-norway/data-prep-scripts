...
def create_vernacular_names_df(raw):
    gunnerus_names_sheet = pd.read_excel('GunnerusVSHoeg-data.xlsx', 'Gunnerus-Names', usecols=['Ataxon', 'VernacularName/s', 'LanguageID', 'GeographicalArea', 'Meaning', 'Comments'], dtype='str', na_values='-')
...
def create_taxon_df(taxa):
    ...
    taxa.replace(' None', '', inplace=True, regex=True)
